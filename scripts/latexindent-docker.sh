...
if ! docker image inspect "$IMAGE" >/dev/null 2>&1; then
    echo "[latexindent-docker] error: docker image not found locally: $IMAGE" >&2
    echo "[latexindent-docker] hint: pull it first with: docker pull $IMAGE" >&2
    exit 1
fi
...
exec docker "${docker_args[@]}" "$IMAGE" latexindent "$@"
