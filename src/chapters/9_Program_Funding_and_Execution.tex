...
\begin{description}
  ...
  \item[\textbf{Who pays what.}] \ac{opn}/\ac{apn}: hardware/software upgrade kits, initial spares; \ac{omn}: installs, fleet introduction/training, minor mods, repair parts consumption; \ac{rdte}: development/test of engineering changes. NAVSUP/\ac{nwcf} activities recover costs via rates for supply/repair.
...
\begin{center}
  \begin{talltblr}[
    ...
    {
      colspec = {F L},
      stretch = 1.2 % (Optional) avoid row Ã¢â‚¬Å“stretchingÃ¢â‚¬Â; see notes below
    }
    \toprule
...
\begin{description}
  ...
  \item[\textbf{\ac{slr}.}] Recover direct labor + overhead + G\&A; shocks flow to NOR/AOR and are managed over time.
