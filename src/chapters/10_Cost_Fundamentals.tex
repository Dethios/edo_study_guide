...
% DESCRIPTION: Cost elements, estimating methods, and learning curves.
% =============================================================================
\begin{document}
...
\begin{description}
  ...
  \item[\textbf{Indirect.}] Pooled and allocated (overhead, \ac{ganda}, facilities, depreciation). See the \ac{far} cost principles~\autocite{FAR}.
\end{description}
...
\end{description}
\note{Use \ac{don}/\ac{dow} cost handbook definitions; be consistent across documents.~\autocite{SECNAV5000-2G,DoDI5000-85}.}
...
\begin{description}
  ...
  \item[\textbf{Parametric (\ac{cer}-based).}] \emph{When:} \ac{aoa} through \ac{tmrr} when historical data supports \acp{cer} (e.g., weight, \acp{sloc}, thrust, kW). \emph{Why:} Enables rapid sensitivity analysis and trade-space exploration with statistical confidence intervals; supports independent cross-checks and early budget phasing.
  ...
  \item[\textbf{Actuals / Extrapolation.}] \emph{When:} From late \ac{tmrr} into production when pilots/\ac{lrip} actuals exist and learning effects are visible. \emph{Why:} Highest fidelity; captures learning curves and process improvements; anchors estimates to realized performance and \ac{csdr} actuals~\autocite{DAU-AAF}.
\end{description}

\begin{longtblr}[
    caption = {Comparison of Estimating Methods}
  ]{
    colspec = {@{} L L L L @{}}
  }
  \toprule
...
Escalate base-year costs to then-year using approved \ac{dow} inflation indices; separate real growth from price growth; align phasing to obligation windows~\autocite{DoDFMR-Vol3}.
