...
\begin{description}
  ...
  \item[\textbf{\acp{rmc}.}] Regional Maintenance Centers deliver voyage repairs, execute \ac{cmp} work, and act as the local \ac{ta} for availabilities; they synchronize contracting, production shops, and total ship readiness assessments (TSRA) under \ac{cnrmc}~\autocite{NAVSEAINST5450-145A,edo-4-1-1-fleet-maint-assets-2025}.
  \item[\textbf{\ac{supship}.}] Supervisors of Shipbuilding administer new construction and conversion/modernization contracts as \acp{aco}, coordinate budget execution, and provide technical oversight for nuclear refueling overhauls (e.g., CVN \ac{rcoh})\FlagBilletNote{RDML Jonathan J.\ Jett-Parmer serves as NAVSEA Deputy Commander, Supervisor of Shipbuilding; RDML Robert J.\ Dodson (USNR) is the reserve deputy}\autocite{NAVSEAINST5450-145A,edo-4-1-1-fleet-maint-assets-2025}.
...
\begin{description}
  \item[\textbf{Flag oversight (EDO).}] \ac{cnrmc} is an EDO flag billet (RDML Dan L.\ Lannamann) reporting to \ac{comnavsea}; CNRMC sets standards and performance goals for all \acp{rmc} and aligns with TYCOM production priorities\FlagBilletNote{\ac{comnavsea} is VADM James P.\ Downey; \ac{cnrmc} is RDML Dan L.\ Lannamann}\autocite{navy-lannamann-2025,NAVSEAINST5450-145A}.
  \item[\textbf{RMC reporting.}] \acp{rmc} (e.g., MARMC, SERMC, SWRMC, PSNS-IMF code, JSRMC, Bahrain) report administratively to \ac{cnrmc} and operationally to the supported TYCOM, with competency-aligned codes (100 fleet support, 200 availability management, 300 production shops, 400 QA/TA) to standardize execution\autocite{NAVSEAINST5450-145A}.
  \item[\textbf{Work control touchpoints.}] \ac{rmmco}, availability work package managers, and contracting officers integrate with ship's force, Port Engineers, and \ac{tycom} maintenance officers; EDO Port Engineers and RMC production officers bridge technical authority to the TYCOM readiness owner\autocite{NAVSEAINST5450-145A,edo-4-1-1-fleet-maint-assets-2025}.
\end{description}
