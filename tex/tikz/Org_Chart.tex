...
\begin{tikzpicture}[
  ...
  \node[orgchart node, below=1cm of potus] (secwar) {SECWAR};
  \node[orgchart node, below=1cm of secwar] (secnav) {SECNAV};
  \node[orgchart node, below=1cm of secnav] (cno) {CNO};

  % Draw connections for the main chain
  \draw[thick] (potus) -- (secwar);
  \draw[thick] (secwar) -- (secnav);
  ...
  \coordinate (cno_bus) at ($(cno.south)+(0,-.75cm)$);
  \draw[thick] (cno.south) -- (cno_bus);
  % Invisible anchor point below the CNO node
