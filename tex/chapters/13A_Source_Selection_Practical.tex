...
% ====================
\begin{document}
\IfSubfilesClassLoaded{\chapter{Source Selection Practical}}{}
...
\begin{description}
  ...
  \item[\textbf{Evaluation criteria.}] Criteria must discriminate among offerors, be tailored to the acquisition, and be communicated in the solicitation's evaluation factors, commonly \ac{rfp} Section~M when the uniform contract format is used, with clear statements of relative importance~\autocite{edo-3-2-5-source-selection-practical-2025,FAR}.
...
\begin{longtblr}
  ...
  \bottomrule
\end{longtblr}
\note{The uniform contract format has \ac{far}~15.204 exceptions, and Part~IV is not physically included in the resulting contract~\autocite{FAR}.}
...
\begin{description}
  ...
  \item[\textbf{SSAC threshold.}] For competitive negotiated \ac{far} Part~15 acquisitions, \ac{dod} source selection procedures require an \ac{ssac} for total estimated value of \$100M or more unless waived or modified by agency procedure~\autocite{edo-3-2-5-source-selection-practical-2025,DoD-Source-Selection-Procedures-2022}.
\end{description}
...
\begin{description}
  ...
  \item[\textbf{Evaluation-factor requirement.}] The solicitation must state the relative importance of all factors and significant subfactors, and specify how non-cost factors compare to cost/price (significantly more important, approximately equal, or significantly less important); this is typically Section~M when the \ac{rfp} uses the uniform contract format~\autocite{edo-3-2-5-source-selection-practical-2025,FAR}.
\end{description}

\subsection{Evaluation Standards and Rating Scales}
\begin{description}
  \item[\textbf{Standards.}] Standards are developed for each factor/subfactor, use words/numbers/colors, and must be clearly defined for the \ac{sseb}; the \ac{rfp} must state factors, significant subfactors, and relative importance, but \ac{far} does not require disclosure of the rating method~\autocite{edo-3-2-5-source-selection-practical-2025,FAR}.
...
\begin{itemize}
  \item For a practical source-selection prompt, first identify the requirement, acquisition phase, contract objective, \ac{rfp} structure, and the decision authority.
  \item Build evaluation factors that discriminate between offers, align to mission value, and state relative importance clearly enough for Section~M.
  \item Keep the rating method consistent: thresholds determine acceptability; objectives and discriminators support tradeoff value.
  \item As \ac{sseb}, evaluate and document strengths, weaknesses, deficiencies, and risk against the stated factors.
  \item As \ac{ssac}, compare proposals and explain the tradeoff in mission terms; as \ac{ssa}, make the source selection decision and document why it is the best value.
  \item Check the trap: an \ac{lpta} answer changes the result only if every mandatory threshold is acceptable and price becomes the deciding factor.
\end{itemize}
