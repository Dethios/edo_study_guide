...
% ====================
\begin{document}
\IfSubfilesClassLoaded{\chapter{Systems Engineering Overview}}{}
...
\begin{description}
  ...
  \item[\textbf{Role of IPTs.}] \acp{ipt} plan, execute, and implement life-cycle decisions~\autocite{edo-3-5-1-systems-engineering-overview-2025}.
...
\begin{description}
  \item[\textbf{Document flow.}] \ac{cba} $\rightarrow$ \ac{icd} $\rightarrow$ \ac{cdd} $\rightarrow$ \ac{cdd} update (legacy \ac{cpd} function) $\rightarrow$ \ac{sep}/\ac{temp} $\rightarrow$ \ac{dte}, early \ac{ote}, and operational-assessment evidence $\rightarrow$ Milestone~C $\rightarrow$ \ac{lrip}/limited deployment $\rightarrow$ \ac{iote}/\ac{lfte} $\rightarrow$ \ac{frp}/full-deployment decision~\autocite{DoDI5000-85,DoDI5000-98,edo-3-3-3-jcids-2025}.
  \item[\textbf{Artifact ownership.}] \ac{cba} by \ac{jcd}/\ac{ccmd}; \ac{icd} by \ac{jcd}/\ac{fcb}; \ac{cdd} and \ac{cdd} updates by the requirements validation authority; \ac{sep} by \ac{pm}; \ac{temp} by \ac{pm}; \ac{dte} by the program's developmental-test team and \ac{cdt}/\ac{ldto}; and \ac{ote}/\ac{iote} by the independent \ac{ota}, with \ac{dote} oversight for covered/listed programs~\autocite{DoDI5000-85,DoDI5000-98,edo-3-3-3-jcids-2025}.
  \item[\textbf{Decision support.}] Each artifact informs specific acquisition decisions: \ac{icd} supports \ac{mdd}; \ac{cdd} supports Milestone~B; the \ac{cdd} update or legacy \ac{cpd}, \ac{sep}, \ac{temp}, \ac{dte}, and early \ac{ote}/operational-assessment evidence support Milestone~C; and \ac{iote}/\ac{lfte} results normally support the later beyond-\ac{lrip}, \ac{frp}, or full-deployment decision~\autocite{DoDI5000-85,DoDI5000-98,edo-3-3-3-jcids-2025}.
\end{description}
