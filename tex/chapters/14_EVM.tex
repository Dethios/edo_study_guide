...
\begin{itemize}
  ...
  \item \ac{dcma} leads \ac{evm} system acceptance, validation, and ongoing surveillance against \ac{ansi}/\ac{eia}-748 and the \ac{dow} \ac{evm} Implementation Guide; \ac{dcaa} supports with accounting system audits.
...
\begin{description}
  ...
  \item[\textbf{\ac{dcma}}:] Serves as the \ac{dow} \ac{evm} Executive Agent by validating, accepting, and surveilling contractor systems, maintaining the official acceptance roster, and analyzing \ac{ipmdar} and schedule submissions (including the 14-point \ac{ims} review) to focus government attention on emerging issues.
...
\begin{itemize}
  ...
  \item Timing: conducted within six months of contract award (or major replanning event) in accordance with the \ac{dow} \ac{evm} Implementation Guide and service policy~\autocite{edo-3-6-1-evm-intro-2025}.
\end{itemize}
