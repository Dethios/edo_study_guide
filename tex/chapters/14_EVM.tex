...
\begin{description}
  \item[\$100 million and above:] Implement the full \ac{ansi}/\ac{eia}-748 standard and ensure the contractor's system is formally validated by \ac{dcma}.
  \item[\$20 million to \$99.9 million:] Implement \ac{ansi}/\ac{eia}-748 guidelines, with validation required when performance indicates risk.
\end{description}
Additional policy considerations follow~\autocite{edo-3-6-1-evm-intro-2025,DoDI5000-85}.
\begin{itemize}
...
\ac{evm} clauses are flowed in the solicitation and award package (\ac{dfars}~252.234-7001 and 252.234-7002) alongside the Contractor Business Systems clause (\ac{dfars}~252.242-7005) to anchor validation and surveillance requirements~\autocite{DFARS}. Compliance expectations follow~\autocite{edo-3-6-1-evm-intro-2025,DoDI5000-85}.
\begin{itemize}
  ...
  \item \ac{dcma} leads \ac{evm} system acceptance, validation, and ongoing surveillance against \ac{ansi}/\ac{eia}-748 and the \ac{dod} \ac{evm} Implementation Guide; \ac{dcaa} supports with accounting system audits.
...
\begin{description}
  ...
  \item[\textbf{DCMA}:] Serves as the \ac{dod} \ac{evm} Executive Agent by validating, accepting, and surveilling contractor systems, maintaining the official acceptance roster, and analyzing \ac{ipmdar} and schedule submissions (including the 14-point \ac{ims} review) to focus government attention on emerging issues.
  \item[\textbf{DCAA}:] Conducts accounting system audits, corroborates cost data, and supports \ac{dcma} during surveillance events and reviews.
  \item[\textbf{ADA}:] Acts as the department-wide focal point for \ac{evm} policy, guidance, and competency management; issues interpretations to maintain consistent application across programs.
...
\begin{itemize}
  ...
  \item Timing: conducted within six months of contract award (or major replanning event) in accordance with the \ac{dod} \ac{evm} Implementation Guide and service policy~\autocite{edo-3-6-1-evm-intro-2025}.
\end{itemize}
