...
\begin{description}
  ...
  \item[\textbf{\ac{dcma}}:] Serves as the \ac{dod} \ac{evm} Executive Agent by validating, accepting, and surveilling contractor systems, maintaining the official acceptance roster, and analyzing \ac{ipmdar} and schedule submissions (including the 14-point \ac{ims} review) to focus government attention on emerging issues.
  ...
  \item[\textbf{\ac{don} Center for \ac{evm}}:] Coordinates applicability determinations with \ac{dasnapb} and \ac{osd}, and serves as the Navy's central point of contact for \ac{evm} matters.
  \item[\textbf{\ac{supship}}:] Performs many \ac{dcma}/\ac{dcaa} surveillance roles for shipbuilding contracts when delegated by the cognizant program office.\FlagBilletNote{See Appendix~\ref{app:edo-flags} for current \ac{supship} flag billet holders}
\end{description}
...
\begin{figure}[H]
  ...
  \caption[\ac{pmb} ``S''-Curve]{\ac{pmb} cost v.s.\ time chart showing the ``S''-curve. \srcCite{edo-3-6-1-evm-intro-2025}.}
  \label{fig:pmb}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption[\ac{pmb} Development Flow]{The development flow for \ac{pmb}. \srcCite{edo-3-6-1-evm-intro-2025}.}
  \label{fig:pmb_dev}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption[\ac{evm} Variances]{\ac{evm} chart showing the independent variables and variances. \srcCite{edo-3-6-2-evm-analysis-2025}}
  \label{fig:evm_var}
\end{figure}
