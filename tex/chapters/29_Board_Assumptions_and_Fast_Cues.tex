...
\begin{description}
  ...
  \item[\textbf{Technical-review trap.}] \ac{sfr} supports the functional baseline, \ac{pdr} supports the allocated baseline, and \ac{cdr} supports the initial product baseline; \ac{srr} reviews requirements but normally is not the baseline-setting review~\autocite{edo-3-3-2-acq-milestones-phases-2025}.
...
\begin{description}
  ...
  \item[\textbf{Competition trigger.}] Does \ac{cica} demand full and open? If not, which FAR~6.302 exception applies, and what level approves the \acs{ja}~\autocite{USC-3201-CICA,FAR}?
  \item[\textbf{JA vs.\ DF.}] \ac{ja} \ac{df}? Know approval levels and posting rules~\autocite{FAR}.
