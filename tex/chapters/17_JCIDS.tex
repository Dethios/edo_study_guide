...
\subsection{Warfighting Acquisition System (JCIDS Replacement)}
\ac{secwar}'s 7--10~Nov~2025 memoranda and Acquisition Transformation Strategy direct the Department to replace legacy \ac{jcids} paperwork with the Warfighting Acquisition System (\ac{was}), an enterprise approach built on Capability Portfolio Management (\ac{cpm}) and Capability Trade Councils that keep requirements, resourcing, and acquisition synchronized \autocite{secwar-was-memo-2025,secwar-acq-transformation-2025}.
\begin{description}[style=nextline,labelsep=0.5em,font=\bfseries]
  \item[\textbf{Key Operational Problems.}] The \ac{jroc} now publishes a short list of \acp{kop}---problem statements tied to the National Defense Strategy---and stops validating Service-level CDDs except where statute forces it, returning day-to-day authority to the Services \autocite{secwar-jcids-reform-2025}.
  \item[\textbf{Mission Engineering \& Integration Activity.}] \ac{uswre} and \ac{uswas} created the \ac{meia} to run rapid experiments, collect live data, and produce mission-thread analyses that replace lengthy JCIDS studies before resource decisions are made \autocite{secwar-jcids-reform-2025,secwar-was-memo-2025}.
  \item[\textbf{Requirements \& Resourcing Alignment Board.}] The new \ac{rrab}, co-chaired by the DepSecWar and \ac{vcjcs}, aligns each prioritized \ac{kop} with \ac{ppbe} guidance, reallocations, and Joint Acceleration Reserve decisions inside the same budget cycle so validated needs show up in the FYDP immediately \autocite{secwar-jcids-reform-2025}.
  \item[\textbf{Joint Acceleration Reserve.}] The \ac{jar} is a centrally managed pool that bridges the ``valley of death'' by funding promising prototypes or industrial-base surges between POM builds; \ac{rrab} recommendations trigger its use when a \ac{kop} solution proves viable \autocite{secwar-jcids-reform-2025}.
  \item[\textbf{Inputs.}] Mission-thread data, kill-chain metrics, threat assessments, PPBE affordability guidance, experimentation results, and digital engineering artifacts flow into each \ac{cpm} cell so that operational problems are quantified before trades begin \autocite{secwar-acq-transformation-2025}.
  \item[\textbf{Core events.}] The DepSecWar and Vice Chairman convene quarterly \ac{cpm} reviews; \acp{pae} call Capability Trade Councils whenever trades exceed delegated cost/schedule/performance thresholds; unresolved items elevate to the Deputies Management Action Group \autocite{secwar-acq-transformation-2025}.
  \item[\textbf{Outputs.}] Portfolio scorecards, capability roadmaps, trade memoranda, and data packages that substitute for the \ac{icd}/\ac{cdd}/\ac{cpd} sequence while still feeding Program Decision Memoranda, issue papers, and Acquisition Program Baselines \autocite{secwar-acq-transformation-2025}.
  \item[\textbf{Key players.}] DepSecWar, VCJCS, \ac{uswas}, \ac{uswre}, \ac{uswc}, \acp{pae}, and functional analytics teams replace the traditional Gatekeeper/\ac{fcb}/\ac{jroc} staffing chain; Capability Trade Councils provide the audit trail for requirement changes \autocite{secwar-acq-transformation-2025}.
  \item[\textbf{Integration with PPBE and the \ac{das}.}] The strategy instructs \ac{uswc} and \ac{uswas} to budget and execute at the portfolio level, letting \acp{pae} move resources inside those controls while simultaneously updating \ac{apb}s and milestone documentation when trades occur \autocite{secwar-acq-transformation-2025}.
  \item[\textbf{Differences from \ac{jcids}.}] \ac{was} relies on live data and executive accountability instead of protracted document staffing, collapses requirements/resourcing/acquisition touchpoints, and uses Capability Trade Councils in lieu of formal Configuration Steering Boards \autocite{secwar-acq-transformation-2025}.
  \item[\textbf{Legislative status.}] The strategy is policy guidance pending FY~2026 NDAA language; during FY~2025 the Department is executing pilot portfolios (munitions industrial base, Project Overmatch) under DepSecWar memoranda while still producing any \ac{jcids} artifacts explicitly mandated by statute \autocite{secwar-acq-transformation-2025}.
\end{description}

\subsection{What EDOs Must Master Under WAS}
\begin{description}[style=nextline,labelsep=0.5em,font=\bfseries]
  \item[\textbf{Time-based metrics.}] Portfolio scorecards now track median days from \ac{kop} identification to \ac{mdd}/IOC; EDOs must baseline their programs against those targets and be ready to discuss what trades are required to shrink the timeline \autocite{secwar-was-memo-2025}.
  \item[\textbf{Capability Trade Councils.}] Expect recurring requests to convene or support these councils with mission-thread data, digital models, and industrial-base options so that requirement, schedule, and cost trades are justified without defaulting to a \ac{jroc} package \autocite{secwar-acq-transformation-2025}.
  \item[\textbf{Experiment-to-budget pipeline.}] Because \ac{meia} experimentation outputs feed the \ac{rrab}, EDOs must deliver credible prototype results (with cost/sustainment implications) fast enough to influence the same \ac{ppbe} build; missing the window means losing \ac{jar} opportunities \autocite{secwar-jcids-reform-2025}.
  \item[\textbf{Security cooperation touchpoints.}] DSCA/DTSA realignment under \ac{uswas} means exportability, tech-release, and allied production plans are now discussed alongside Capability Trade Councils---EDO briefs should cover how security cooperation choices affect industrial-base capacity and delivery risk \autocite{secwar-arms-transfer-2025}.
\end{description}
