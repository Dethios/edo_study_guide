...
% TODO: Insert Joint Capabilities Integration and Development System to Defense Acquisition System relationship figure (showing validation feeding milestones).
\begin{figure}
	\centering
	\fbox{\parbox{0.9\linewidth}{\centering TODO: Insert Joint Capabilities Integration and Development System to Defense Acquisition System relationship figure (showing validation feeding milestones).}}
	\caption{Relationship between the Joint Capabilities Integration and Development System and the Defense Acquisition System.}
\end{figure}
...
%  TODO: Mermaid flow --- Strategic guidance $\rightarrow$ Joint concepts $\rightarrow$ Mission threads $\rightarrow$ capability-based assessment $\rightarrow$ DOTMLPF-P recommendations $\rightarrow$ Validated requirement $\rightarrow$ DAS milestone.
\begin{figure}
	\centering
	\fbox{\parbox{0.9\linewidth}{\centering TODO: Mermaid flow --- Strategic guidance $\rightarrow$ Joint concepts $\rightarrow$ Mission threads $\rightarrow$ capability-based assessment $\rightarrow$ DOTMLPF-P recommendations $\rightarrow$ Validated requirement $\rightarrow$ DAS milestone.}}
	\caption{Concept-to-requirement flow that must stay synchronized with acquisition execution.}
\end{figure}
...
\begin{description}
	\item[\textbf{Strategic guidance.}] The Secretary of Defense and \ac{cjcs} translate national objectives into force design problems and assign lead sponsors; the Joint Staff J-7 houses concept development and readiness assessments that set the analytical agenda~\autocite{jcids-manual-2021}.
...
% TODO: Insert Joint Capabilities Integration and Development System within Defense Acquisition System figure showing ICD$\rightarrow$Milestone~A, CDD$\rightarrow$Milestone~B, CDD Update/CPD$\rightarrow$Milestone~C.
\begin{figure}
	\centering
	\fbox{\parbox{0.9\linewidth}{\centering TODO: Insert Joint Capabilities Integration and Development System within Defense Acquisition System figure showing ICD$\rightarrow$Milestone~A, CDD$\rightarrow$Milestone~B, CDD Update/CPD$\rightarrow$Milestone~C.}}
	\caption{Placement of Joint Capabilities Integration and Development System artifacts inside the phased Major Capability Acquisition pathway.}
\end{figure}
...
\begin{description}
	...
	\item[\textbf{Leadership \& education.}] Insert leadership development or acquisition workforce upskilling when governance bottlenecks (e.g., \ac{jroc} literacy) create the gap.
...
\subsection{Current Events: Secretary of War Acquisition Transformation Strategy}
The 10~Nov~2025 Acquisition Transformation Strategy directs the Warfighting Acquisition System to eliminate legacy \ac{jcids} bureaucracy, convert Configuration Steering Boards into \acp{ctctrade}, and empower \acp{pae} to make requirements trades inside cost/schedule thresholds~\autocite{secwar-acq-transformation-2025}. The memo explicitly calls for \ac{jcids} elimination and streamlining of \ac{das} reviews, meaning Navy teams must plan for near-term transition periods where \acp{pae} convene \acp{ctctrade} to validate trades in lieu of traditional Joint Staff staffing, while \ac{das} documentation is simultaneously pushed to lower approval levels~\autocite{secwar-acq-transformation-2025}. Expect joint governance updates (Gatekeeper rules, \ac{fcb} charters) and plan to capture trade rationales in Decision Memoranda so that future auditors can trace how requirements changes affected \acp{apb}.

\subsection{Practitioner Steps (Board-Focused)}
\begin{enumerate}
	\item Confirm the latest Joint Staff validation status in \ac{km-ds} before taking any \ac{das} decision memo to \ac{asnrdanda}---out-of-date \ac{jsd} codes are a fast way to fail a board question~\autocite{edo-3-3-3-jcids-2025}.
	\item Map every \ac{kpp}/\ac{ksa} to a test event and logistic metric inside the \ac{sep}/\ac{temp}; if a \ac{kpp} lacks a verification method, the Gatekeeper will bounce the package~\autocite{DoDI5000-85}.
	\item Show which \ac{dotmlpfp} levers were exhausted prior to requesting a new-start line; Fleet leadership expects to see realistic non-materiel mitigations even when you are asking for ships or software~\autocite{jcids-manual-2021}.
\end{enumerate}

\subsection{Checklist and Decision Trace}
\begin{description}
	\item[\textbf{Checklist.}] (1) Latest \ac{icd}/\ac{cdd}/\ac{cpd} and Gatekeeper Control Number on file; (2) Net-Ready, cybersecurity, safety, and intel certifications aligned with milestone schedule; (3) \ac{dotmlpfp} changes resourced; (4) Navy Gate packages (Gate~0/1/2) consistent with joint validation status.
	\item[\textbf{Decision trace.}] If the capability gap is still unacceptable after \ac{dotmlpfp} adjustments, elevate to the appropriate validation authority with quantified risk; once validated, lock the \ac{kpp}s into the \ac{apb} and document all trades via \ac{ctctrade} or \ac{csb} minutes so the \ac{das} record remains synchronized with the requirements record~\autocite{jcids-manual-2021,DoDI5000-85}.
\end{description}
