...
\begin{description}[style=nextline,labelsep=0.5em,font=\bfseries]
  ...
  \item[\textbf{Key players.}] DepSecWar, VCJCS, USD(A\&S), USD(R\&E), USD(C), \acp{pae}, and functional analytics teams replace the traditional Gatekeeper/\ac{fcb}/\ac{jroc} staffing chain; Capability Trade Councils provide the audit trail for requirement changes.\autocite{secwar-acq-transformation-2025}
  \item[\textbf{Integration with PPBE and the \ac{das}.}] The strategy instructs USD(C) and USD(A\&S) to budget and execute at the portfolio level, letting \acp{pae} move resources inside those controls while simultaneously updating \ac{apb}s and milestone documentation when trades occur.\autocite{secwar-acq-transformation-2025}
